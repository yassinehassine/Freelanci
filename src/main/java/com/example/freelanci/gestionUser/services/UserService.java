...
public class UserService {
    ...
    // Custom method to filter users by role
    public List<User> filterByRole(User.UserRole role) {
        if (role == null) {
            return userRepository.findAll();  // Si aucun rôle n'est spécifié, on retourne tous les utilisateurs
        }
        return userRepository.findByRole(role);  // Filtrer par rôle
    }
}
